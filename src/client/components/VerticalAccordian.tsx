...
const VerticalAccordion = () => {
  ...
  return (
    <section className="bg-indigo-600 p-4">
      <div className="lg:flex-row lg:h-[450px] mx-auto flex h-fit w-full max-w-6xl flex-col overflow-hidden shadow">
        {items.map((item) => {
          return (
            <Panel
              key={item.id}
...
const Panel = ({ open, setOpen, id, Icon, title, imgSrc, description }) => {
  ...
  return (
    <>
      <button
        className="lg:flex-col group relative flex flex-row-reverse items-center justify-end gap-4 border-b-[1px] border-r-[1px] border-slate-200 bg-white p-3 transition-colors hover:bg-slate-50"
        onClick={() => setOpen(id)}
      >
        <span
          style={{
          ...
          className="lg:block hidden rotate-180 text-xl font-light"
        >
          {title}
        </span>
        <span className="lg:hidden block text-xl font-light">{title}</span>
        <div className="lg:w-full grid aspect-square w-6 place-items-center bg-indigo-600 text-white">
          <Icon />
        </div>
        <span className="lg:border-b-0 lg:border-t-[1px] lg:bottom-[50%] lg:right-0 absolute bottom-0 right-[50%] z-20 h-4 w-4 translate-x-[50%] translate-y-[50%] rotate-45 border-b-[1px] border-r-[1px] border-slate-200 bg-white transition-colors group-hover:bg-slate-50" />
      </button>
